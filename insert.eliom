...
{client{
  ...
  let create url_input button user_container logged_user =
  ...
  let process_res t res =
   ...
   match (Utils.process_event_answer input_text res) with
      ...
      | `Ok event -> begin
        ...
        lwt res = Login.logged_process_all_rsvp t.logged_user input_text in
   ...
   let on_url_input_changes t _ _ =
     ...
     try_lwt begin
       match_lwt %rpc_event_exists input_text with
         | None -> begin
           lwt res = Login.logged_lwt_api t.logged_user input_text in
