...
{client{
  ...
  let process_res res url_input user_container_elt to_insert button_dom =
    match (Utils.process_event_answer url_input res) with
      | `Err x -> begin
        Html5.Manip.replaceChildren user_container_elt x;
        Lwt.return_unit end
      | `Ok event -> begin
        let attending_s, declined_s, invited_s =
         Utils.make_wait_user_container (), Utils.make_wait_user_container (), Utils.make_wait_user_container ()
        in
        ...
        Html5.Manip.replaceChildren user_container_elt [displayed_table];
...
let insert_service unused unused2 =
      ...
      async (fun () ->
        ...
        ignore(changes url_input_dom
                 (fun _ _ ->
                   ...
                   Html5.Manip.replaceChildren %user_container_elt [];
                   ...
                   try_lwt begin
                     match_lwt %rpc_event_exists url_input with
                       | None -> begin
                         ...
                         lwt () = process_res res url_input %user_container_elt to_insert button_dom in
                         Lwt.return_unit
                       end
                       | Some event -> begin
                         let res = display_event_already_inserted event in
                         Html5.Manip.replaceChildren %user_container_elt [div ~a:[a_style "text-align:center"] res];
                         Lwt.return_unit
                       end
                   end
                   with x -> (Html5.Manip.replaceChildren %user_container_elt
  ...
  Lwt.return (Eliom_tools.D.html ~title: "insert an url"
                ...
                (body [Utils.fb_root_div;
                       ...
                       div ~a:[a_class ["container"]] [user_container_elt]])
